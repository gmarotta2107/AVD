...
class BasicAgent(object):
    ...
    def _affected_by_stop_sign(self, stop_sign_list=None, max_distance=None):
        ...
        Method to check if there is a stop sign affecting the vehicle.
            ...
            :param stop_sign_list (list of carla.TrafficLight): list containing TrafficLight objects.
                If None, all traffic lights in the scene are used
            :param max_distance (float): max distance for traffic lights to be considered relevant.
                If None, the base threshold value is used
        """
        # if ignore setting is set to True, simply return (False, None)
        if self._ignore_stop_signs:
            return (False, None, -1)

        # if no list is passed to the function, take manually all the traffic lights actors in the world
        if not stop_sign_list:
            stop_sign_list = self._world.get_actors().filter("*stop*")

        # if no max_distance is passed to the function, take the default threshold for stopping sign distance
        if not max_distance:
        ...
        for stop_sign in stop_sign_list:
            # getting trigger waypoint of the stop sign
            ...
            # if the stop sign is too far from the ego vehicle, just ignore it
            if trigger_wp.transform.location.distance(ego_vehicle_location) > max_distance:
                continue

            # if the stop sign is on a different road with respect to the ego vehicle, just ignore it
            ...
            # stop sign is in the opposite sense with respect to the ego vehicle
            if dot_ve_wp < 0:
                continue

            # checking if the stop sign is within distance with respect to the ego vehicle
            if is_within_distance(trigger_wp.transform, self._vehicle.get_transform(), max_distance, [0, 90]):
                self._last_stop_sign = stop_sign
                distance = compute_distance(self._vehicle.get_transform().location, trigger_wp.transform.location)
                return (True, stop_sign, distance)

        # no stop sign affect the ego vehicle
        return (False, None, -1)
    

    
    def resume_driving(self, speed=None, debug=False):
        """
        Riparte con throttle>0:
          - se passo `speed` (km/h), aggiorno anche il target del planner
          - richiedo al local_planner un nuovo VehicleControl 
        """
        # 1) se voglio cambiare target
        if speed is not None:
            self.set_target_speed(speed)
        # 2) genero il controllo: qui il brake sarÃ  automaticamente a 0
    ...
    def _biker_obstacle_detected(self, bike_list=None, max_distance=None, up_angle_th=90, low_angle_th=0, lane_offset=0):
        """
        Method to detect bikers blocking the path or intending to cross the road
            ...
            return: sorted list contained all bikers or a list with a single element that indicates that is not a biker
        """
        ...
        for target_bike in bike_list:
            same_lane = True
            target_transform = target_bike.get_transform()
            target_wpt = self._map.get_waypoint(target_transform.location, lane_type=carla.LaneType.Any)

            # checking if at least one of the vertices of the bounding box is in the same road id as
            # the ego vehicle
            target_bike_bb = target_bike.bounding_box.get_world_vertices(target_transform)
            is_in_same_lane_id = False
            for i, point_location in enumerate(target_bike_bb):
                if (self._map.get_waypoint(point_location, lane_type=carla.LaneType.Any).lane_id == (ego_wpt.lane_id + lane_offset)):       #controllo sse si trova sulla stessa corsia di ego
            ...
            if not ego_wpt.is_junction or not target_wpt.is_junction:
                ...
                if target_wpt.road_id != ego_wpt.road_id or (target_wpt.lane_id != ego_wpt.lane_id  + lane_offset and target_wpt.lane_id != ego_wpt.get_right_lane().lane_id + lane_offset):
                    # next_wpt = self._local_planner.get_incoming_waypoint_and_direction(steps=3)[0]
                    if not next_wpt:
                        continue
                    if target_wpt.road_id != next_wpt.road_id \
                        or (target_wpt.lane_id != next_wpt.lane_id  + lane_offset and target_wpt.lane_id != next_wpt.get_right_lane().lane_id + lane_offset):
                        continue

                target_forward_vector = target_transform.get_forward_vector()
                target_extent = target_bike.bounding_box.extent.x
                ...
                if is_within_distance(target_rear_transform, ego_front_transform, max_distance, [low_angle_th, up_angle_th]):
                    if target_wpt.lane_id != next_wpt.lane_id and not is_in_same_lane_id:
                        same_lane = False

                    temp_bike_list.append((True, target_bike, compute_distance(target_transform.location, ego_transform.location), same_lane))
            ...
            # Waypoints aren't reliable, check the proximity of the vehicle to the route
                ...
                target_extent = target_bike.bounding_box.extent.x
                if target_bike.id == self._vehicle.id:
                    continue
                if ego_location.distance(target_bike.get_location()) > max_distance:
                    continue

                target_bb = target_bike.bounding_box
                target_vertices = target_bb.get_world_vertices(target_bike.get_transform())
                target_list = [[b.x, b.y, b.z] for b in target_vertices]
                target_polygon = Polygon(target_list)

                if ego_polygon.intersects(target_polygon):
                    temp_bike_list.append((True, target_bike, compute_distance(target_bike.get_location(), ego_location), False))

        if len(temp_bike_list) > 0:
            return sorted(temp_bike_list, key=lambda x: x[2])
        else:
            return [(False, None, -1, None)]
    ...
    def _walker_obstacle_detected(self, walker_list=None, max_distance=None, up_angle_th=90, low_angle_th=0, lane_offset=0):
        """
        Method to detect pedestrians blocking the path or intending to cross the road

            :param walker_list (list of carla.Walker): list contatining walker objects.
                If None, all vehicle in the scene are used
            :param max_distance: max freespace to check for obstacles.
                If None, the base threshold value is used

            return: sorted list contained all walkers or a list with a single element that indicates that is not a walker
        """
        ...
        for target_walker in walker_list:
            ...
            # checking if at least one of the vertices of the bounding box is in the same road id as
            # the ego vehicle
            target_walker_bb = target_walker.bounding_box.get_world_vertices(target_transform)
            is_in_same_lane_id = False
            for i, point_location in enumerate(target_walker_bb):
                if (self._map.get_waypoint(point_location, lane_type=carla.LaneType.Any).lane_id == (ego_wpt.lane_id + lane_offset)):
                    is_in_same_lane_id = True
                    break
            ...
            if not ego_wpt.is_junction or not target_wpt.is_junction:
                ...
                target_forward_vector = target_transform.get_forward_vector()
                target_extent = target_walker.bounding_box.extent.x
                target_rear_transform = target_transform
                target_rear_transform.location -= carla.Location(
                    x=target_extent * target_forward_vector.x,
                    y=target_extent * target_forward_vector.y,
                ...
                if is_within_distance(target_rear_transform, ego_front_transform, max_distance, [low_angle_th, up_angle_th]):
                    ...
                    temp_walker_list.append((True, target_walker, compute_distance(target_transform.location, ego_transform.location), same_lane))
            ...
            # Waypoints aren't reliable, check the proximity of the vehicle to the route
                ...
                if target_walker.id == self._vehicle.id:
                    continue
                if ego_location.distance(target_walker.get_location()) > max_distance:
                    continue
                ...
                if ego_polygon.intersects(target_polygon):
                    temp_walker_list.append((True, target_walker, compute_distance(target_walker.get_location(), ego_location), False))

        if len(temp_walker_list) > 0:
            return sorted(temp_walker_list, key=lambda x: x[2])
        else:
            return [(False, None, -1, None)]
    ...
    def _vehicle_obstacle_in_junction_detected(self, vehicle_list=None, max_distance=None, up_angle_th=90, low_angle_th=0, lane_offset=0):
        """
        Method to check if there is a vehicle in a junction in front of the agent blocking its path.
