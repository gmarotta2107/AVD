...
class LocalPlanner(object):
    ...
    def get_junction_waypoint(self, steps=30):
        """
        Returns direction and waypoint at a distance ahead defined by the user.

            :param steps: number of steps to get the incoming waypoint.
        """
        junction_waypoint_list = []
        is_empty = True

        for i in range(steps):
            wpt, _ = self._waypoints_queue[i]
            if wpt.is_junction:
                junction_waypoint_list.append(wpt.id)
        
        return junction_waypoint_list
    
    def get_current_waypoint(self):
        if len(self._waypoints_queue) > 0:
            wpt, _ = self._waypoints_queue[0]
            return wpt
        else:
            return None
